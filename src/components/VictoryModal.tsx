...
export const VictoryModal: React.FC<VictoryModalProps> = ({
  ...
  const selectedBonusSkin = bonus?.selectionSkinId === undefined
    ? undefined
    : champion.skins.find(candidate => candidate.id === bonus.selectionSkinId);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm transition-opacity duration-300 animate-fade-in" data-testid="victory-modal">
      <div
        ...
        className={`w-full max-w-md bg-[#1e2328] border-2 ${isSurrendered ? 'border-rose-600/70 shadow-[0_0_40px_rgba(225,29,72,0.3)]' : 'border-[#c8aa6e] shadow-[0_0_40px_rgba(200,170,110,0.35)]'} rounded-2xl overflow-hidden relative animate-flip-in`}
      >
        <button
          ref={closeButtonRef}
          type="button"
        ...
        <div className="relative h-44 sm:h-48 overflow-hidden bg-[#091428]">
          {!imageError ? (
            <img
              src={usingFallback ? fallbackImage : primaryImage}
              alt={champion.name}
              onError={() => {
            ...
            <div className="w-full h-full flex items-center justify-center text-[#a09b8c] text-sm">Image unavailable</div>
          )}
          ...
          <div className="absolute bottom-3 left-0 right-0 text-center px-4">
            {isSurrendered && <span className="text-[11px] uppercase tracking-wider text-rose-300 font-bold block mb-0.5">The answer was</span>}
            <h3 id="victory-modal-title" className="text-2xl sm:text-3xl font-bold font-serif text-[#f0e6d2] tracking-wide drop-shadow">
              {champion.name}
            </h3>
            {mode === 'ability' && abilityKey && (
              <div className="mt-1 inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-[#091428]/85 border border-[#c8aa6e]/40 text-xs text-[#f0e6d2] backdrop-blur shadow-sm">
                ...
                <span>{champion.abilities.find(a => a.key === abilityKey)?.name || 'Ability'} ({abilityKey === 'P' ? 'Passive' : `Key: ${abilityKey}`})</span>
                {bonus && bonus.status !== 'pending' && (
                  <span className={`font-bold ml-1 ${bonus.status === 'correct' ? 'text-emerald-400' : bonus.status === 'skipped' ? 'text-[#c8aa6e]' : 'text-rose-400'}`}>
                    {bonus.status === 'correct' ? '✓ Bonus' : bonus.status === 'skipped' ? 'Skipped' : '✗ Missed'}
                  </span>
                )}
              </div>
            )}
            {mode === 'splash' && bonus && bonus.status !== 'pending' && (
              <div className="mt-1 inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-[#091428]/85 border border-[#c8aa6e]/40 text-xs text-[#f0e6d2] backdrop-blur shadow-sm">
                <ImageIcon className="w-3 h-3 text-amber-400" />
                <span>
                  {bonus.status === 'correct'
                    ? `Bonus skin: ${selectedBonusSkin?.name || 'Correct'}`
                    : bonus.status === 'missed'
                      ? `Bonus missed${selectedBonusSkin ? `: ${selectedBonusSkin.name}` : ''}`
                      : 'Bonus skipped'}
                </span>
              </div>
            )}
        ...
        <div className="p-5 sm:p-6 text-center">
          <div className="flex items-center justify-center gap-6 my-1">
            <div className="flex flex-col items-center">
              ...
              <span className="text-xs text-[#a09b8c] uppercase tracking-wider">{guessCount === 1 ? 'Guess' : 'Guesses'}</span>
            </div>
          ...
          <div className="flex flex-col gap-2.5 mt-5">
            {isUnlimited ? (
              <button type="button" onClick={onNextRound} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#c8aa6e] to-[#785a28] hover:from-[#e0c488] hover:to-[#967032] text-black font-bold py-3 px-6 rounded-xl shadow-lg transition-all">
                <span>Play Next Champion</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            ) : (
              <button type="button" onClick={onShare} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-500 hover:to-teal-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg transition-all">
            ...
            <div className="mt-2 pt-3 border-t border-[#785a28]/30">
              <span className="text-[11px] uppercase tracking-wider font-semibold text-[#a09b8c] block mb-2">Or Switch To Another Mode:</span>
            ...
            {isUnlimited && (
              <button type="button" onClick={onShare} className="w-full flex items-center justify-center gap-1.5 text-[#a09b8c] hover:text-[#f0e6d2] text-xs transition py-1 mt-1">
